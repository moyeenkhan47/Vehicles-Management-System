...
<body>
	...
	<c:choose>
		<c:when test="${!empty eftInitiated}">
			<div class="row">
				<div class="col-lg-12">
							...
							<div class="table-responsive" style="margin-bottom: 10%;">
								...
								<form method="POST" id="form">
									...
									<table id="myDatatable1"
										...
										<tbody>
											<c:forEach items="${eftInitiated}" var="file">
												<tr>
													
													<td><input type="checkbox" value="${file.PYM02BID}"
														name="batchIds"></td>
         ...
         dateStyle = "medium" timeStyle = "long" value = "${file.uploadDate}" pattern="d MMM y hh:mm a" /></td> --%>
													<td><fmt:formatDate type="both" dateStyle="medium"
															value="${file.PYM02DATU}" pattern="d MMM y" /></td>
													<td><c:out value="${file.PYM02TOT}" /></td>
													<td><c:out value="${file.PYM02TAMT}" /></td>
													<td><c:if test="${file.PYM02STS=='I'}">
														Initiated
													</c:if> <c:if test="${file.PYM02STS=='R'}">
														Rejected
													</c:if> <c:if test="${file.PYM02STS=='A'}">
														Approved
													</c:if></td>
													<td><c:out value="${file.PYM02BID}" /></td>
													
													<td><a class="btn btn-primary"
														href="${pageContext.request.contextPath}/eft/fileUploadData/${file.PYM02BID}.do">View
															Details</a> &nbsp;&nbsp;
												</tr>
											</c:forEach>
										</tbody>
									...
									<tr><td></td>
									<td><input type="button" value="Reject" class="btn-space btn btn-primary" id="reject" disabled="disabled"/>
									<input type="button" value="Approve" class="btn-space btn btn-primary" id="approve" disabled="disabled"/></td>
									</tr>
									</table>
							...
							$("#approve")
									.click(
											function() {
												...
												//alert(base_url+ '/eft/approveRejectFile.do');
												$('#form')
														.attr(
																'action',base_url+
																'/eft/approveRejectFile.do');
												$("#form").submit();
											});
