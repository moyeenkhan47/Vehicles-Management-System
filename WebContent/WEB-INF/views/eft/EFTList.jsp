...
<h3>Filter Record</h3>
    ...
    margin-left: 100px;" >
		...
		action="${pageContext.request.contextPath}/eft/search.do" 
		commandName="eftSearch">	
		...
		<table class ="table table-bordered table-striped table-highlight" >
			...
			<tr>
				<td>Payment Purpose Code</td>
				<td><form:select path="paymentPurposeCode" >
						<option value="0">ALL</option>						
						<form:options items="${purposeCodeList}"  />						
					</form:select>
				</td>
...
<c:choose>
	...
	<h3 style="text-align:center;"> Search Results</h3>
		...
		action="${pageContext.request.contextPath}/eft/submitFile.do">    	
			...
			<tbody>
				<c:forEach items="${fileList}" var="file">
					<tr>
						<td>
						<c:if test="${file.PYM02STS=='R'}" >
						<input type="checkbox" value="${file.PYM02BID}" name="batchIds">
						</c:if>
						</td>
						<td><c:out value="${file.PYM02FIL}" /></td>
						<td><c:out value="${file.PYM02USRU}" /></td>
					<%-- 	<td>
						 <fmt:formatDate type = "both" 
         dateStyle = "medium" timeStyle = "long" value = "${file.uploadDate}" pattern="d MMM y hh:mm a" /></td> --%>
         	<td>
						 <fmt:formatDate type = "both" 
         dateStyle = "medium" value = "${file.PYM02DATU}" pattern="d MMM y" /></td> 
						<td><c:out value="${file.PYM02TOT}" /></td>
					<%--Added formatNumber for Amount on 6 Aug 2017 --%>
						<td> <fmt:formatNumber value="${file.PYM02TAMT}"  minFractionDigits="2"/> </td>
						
						<td>
						<c:if test="${file.PYM02STS=='I'}" >
							Initiated
						</c:if>
						<c:if test="${file.PYM02STS=='R'}" >
							Rejected
						</c:if>
						<c:if test="${file.PYM02STS=='A'}" >
							Approved
						</c:if>
						
						</td>
						<td><c:out value="${file.PYM02BID}" /></td>	
						<td><c:out value="${file.PYM02REM}" /></td>									
						<td>
						<a class="btn btn-primary" href="${pageContext.request.contextPath}/eft/fileUploadData/${file.PYM02BID}.do">View Details</a>&nbsp;&nbsp;						
...
<script>
	$(document).ready(function() {
		...
		$("input[name=batchIds]").click(function(){
			var checked = $("input[name='batchIds']:checked").length;
