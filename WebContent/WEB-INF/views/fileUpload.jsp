...
function checkFileName(){
	...
	filename = filename.substring(filename.lastIndexOf("\\")+1);
		...
		$.ajax({
			type : "post",
			url : base_url + "/upload/checkFileName.do",
			cache : false,
...
<form id="form" class="form-inline" method="post" action="${pageContext.request.contextPath}/upload/form.do" enctype="multipart/form-data" >
...
<p style="color: green;text-align: left;" id="message">${successMessage}</p>
    ...
    <div class="form-group">
      <label for="email">Please Choose file:   </label>
      <input type="file" class="form-control" id="file" name="file" placeholder="Enter email" accept=".xls,.xlsx"/>
    </div>
