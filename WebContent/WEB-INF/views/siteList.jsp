...
<body>		
	...
	<a  class="btn btn-primary btn-lg pull-right" id="primarybtn" href="${pageContext.request.contextPath}/site/showAddSites.do">Add Site</a>
    </div>
	<h3 id="siteh3">Sites</h3>
	<span style="color: green;text-align: center;display:inline-block;margin-left:12%;">${message}</span>
	<span style="color: red;text-align: center;display:inline-block;margin-left:20%;">${Errormessage}</span>
	               </div>
					...
					<div class="panel-body">
						<div class="table-responsive">
							<table id="myDatatable"	class="display table table-bordered table-striped table-hover">
								<thead>
									<tr>
										<th><span class="spn">Site ID</span></th>
										<th><span class="spn">Site Name</span></th>
										<th><span class="spn">Site Location</span></th>
										<th><span class="spn">Construction type</span></th>
										<th><span class="spn">Start Project Date</span></th>
										 <th><span class="spn">End Project Date</span></th>
										<!--  <th><span class="spn">Start Date</span></th>
										 <th><span class="spn">End Date</span></th>
										 <th><span class="spn">Stage</span></th>
										 -->  <th><span class="spn">Status</span></th>
										  <th><span class="spn">Action</span></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${siteList}" var="site" varStatus="i">
										<tr>
											...
											<td><c:out value="${site.siteLocation}" /></td>
											<td><c:out value="${site.construction_type}" /></td>
											<td><c:out value="${site.pro_Start_Date}" /></td>
											 <td><c:out value="${site.pro_End_Date}" /></td>
											<%--<td><c:out value="${site.start_Date}" /></td>
											<td><c:out value="${site.end_Date}" /></td>
											 <td><c:out value="${site.stage}" /></td>
											--%>	<td><c:out value="${site.status}" /></td>
											<%-- <td><c:out value="${site.siteSupervisor}" /></td>
				 --%>
											<td><a class="btn1" data-toggle="modal" data-target="#myModal" title="View"
												href="${pageContext.request.contextPath}/site/viewSite/${site.siteId}.do"><i class="fa fa-eye" style="font-size:18px"></i></a>&nbsp;&nbsp;
												<a class="btn1" title="Edit"
												href="${pageContext.request.contextPath}/site/ShowUpdateSite/${site.siteId}.do"><i class="fa fa-edit" style="font-size:18px"></i></a>&nbsp;&nbsp;
												 <c:if test="${sessionScope.User.role=='1'}">
												<a class="btn1" title="Delete" href="${pageContext.request.contextPath}/site/deleteSite/${site.siteId}.do"
												onclick="getEntryDtl(${i.index+1})"><i class="fa fa-trash-o" style="font-size:18px;color:red"></i></a></c:if>
												<input type="hidden" id="siteid_${i.index+1}" value="${site.siteId}">
												</td>
										</tr>
									</c:forEach>
								</tbody>
 ...
 <script type="text/javascript" src="${pageContext.request.contextPath}/vendor/datatableJsCss/js/buttons.print.min.js"></script>
	...
	function getEntryDtl(id){
		getEntry(id)
	}
	function getEntry(id){
		var base_url = window.location.origin;	
		base_url = base_url+"<%=request.getContextPath()%>";
		var siteid=$("#siteid_"+id).val();
		siteid = ""==siteid ?0:siteid; 
		console.log(siteid);
			$.ajax({
				type : "post",
				url : base_url + "/dataEntry/checkEntry.do",
				cache : false,
				data : 'siteid='+siteid,
